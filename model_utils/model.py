...
class FullyCNNSEModelV3(object):
    ...
    def encode(self, x):
        self.encode_1 = conv_bn_relu(x, 10, kernel_size=(1, 1), stride=(1, 1), is_training=self.is_training, padding="Valid", scope="encode_1")
        self.encode_2 = separable_conv(self.encode_1, 12, kernel_size=(8, 6), stride=(1, 1), is_training=self.is_training, scope="encode_2")
        self.encode_3 = separable_conv(self.encode_2, 14, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="encode_3")
        self.encode_4 = separable_conv(self.encode_3, 15, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="encode_4")
        self.encode_5 = separable_conv(self.encode_4, 19, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="encode_5")
        self.encode_6 = separable_conv(self.encode_5, 21, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="encode_6")
        self.encode_7 = separable_conv(self.encode_6, 23, kernel_size=(1, 7), stride=(1, 1), is_training=self.is_training, scope="encode_7")
        encode_8 = separable_conv(self.encode_7, 25, kernel_size=(1, 11), stride=(1, 1), is_training=self.is_training, scope="encode_8")
        return encode_8

    def decode(self, x):
        x = separable_conv(x, 23, kernel_size=(1, 7), stride=(1, 1), is_training=self.is_training, scope="decode_1", skip_input=self.encode_7)
        x = separable_conv(x, 21, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="decode_2", skip_input=self.encode_6)
        x = separable_conv(x, 19, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="decode_3", skip_input=self.encode_5)
        x = separable_conv(x, 15, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="decode_4", skip_input=self.encode_4)
        x = separable_conv(x, 14, kernel_size=(1, 5), stride=(1, 1), is_training=self.is_training, scope="decode_5", skip_input=self.encode_3)
        x = separable_conv(x, 12, kernel_size=(8, 7), stride=(1, 1), is_training=self.is_training, scope="decode_6", skip_input=self.encode_2)
        x = separable_conv(x, 10, kernel_size=(1, 11), stride=(1, 1), is_training=self.is_training, scope="decode_7", skip_input=self.encode_1)
        x = conv_bn_relu(x, 1, kernel_size=(1, 15), stride=(1, 1), is_training=self.is_training, scope="decode_8", use_norm=False, use_act=False)
        return x
